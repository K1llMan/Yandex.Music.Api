    ...
    public class TestAuthApplication
    {
            ...
            while (true)
            {
                ...
                switch (key.Key)
                {
                    ...
                    case ConsoleKey.Enter:
                        MenuItem selectedItem = _currentItems[_selectedIndex];
        ...
        private void HandlePhoneLoginProcess()
        {
            ...
            try
            {
                ...
                YValidatePhoneNumberResult? phoneResult = _yandexMusicClient.PassportValidatePhoneNumber(login);
                ...
                YCheckAvailabilityResult? phoneAvailability = _yandexMusicClient.PassportCheckPhoneAvailability(phoneResult.PhoneNumber.International);
                ...
                YSendPushResult? sendPushResult = _yandexMusicClient.PassportSuggestSendPush(phoneResult.PhoneNumber.International);
                if (sendPushResult.AppsForBrightPush is not null && sendPushResult.AppsForBrightPush.Count > 0)
                {
                    ...
                    YValidateSquatter? validateResult = _yandexMusicClient.PassportValidateSquatter(phoneResult.PhoneNumber.International);

                    if (validateResult.SuggestBy == "s")
                    {
                        YSuggestByPhoneResult? sr = _yandexMusicClient.PassportSuggestByPhone();
                        YMultistepStart? passportUser = _yandexMusicClient.PassportMultistepStart(sr.Accounts.First().Login);
                        if (passportUser is not null)
                        {
                            ...
                            bool isauthorized = AuthentificateByPassword(_session);
        ...
        private void HandleLoginProcess()
        {
            ...
            {
                _yandexMusicClient.PassportCreateAuthSession();
                YMultistepStart? passportUser = _yandexMusicClient.PassportMultistepStart(login);
            ...
            MenuItem twoFactorMenu = new MenuItem($"🔐 2FA для {_session.Login}");
            twoFactorMenu.SubItems.Add(new MenuItem("📨 Войти по Паролю", session =>
            {
                bool isauthorized = AuthentificateByPassword(session);
            ...
            {
                bool isauthorized = AuthentificateBySms(session);
            ...
            foreach (MenuItem item in twoFactorMenu.SubItems)
            {
        ...
        private void DrawBreadcrumbs()
        {
            ...
            MenuItem? current = _currentParent;
        ...
        private bool AuthentificateByPassword(UserSession session)
        {
            ...
            YPassportUser? passwordResponse = _yandexMusicClient.PassportAuthByPassword(inputPassword);
        ...
        private void CreateAccessToken(UserSession session)
        {
            ...
            YPassportSession? passportSession = _yandexMusicClient.PassportGetSession();
            if (string.IsNullOrWhiteSpace(passportSession.DefaultUid))
            {
                ShowMessage("Сессия не создана", _errorColor);
            }

            YPassportSessionStatus? sessionStatus = _yandexMusicClient.PassportGetSessionStatus();
            ...
            YAccessToken? authToken = _yandexMusicClient.GetTokenBySession();
